...
$urlback = (isset($_GET['urlback'])) ? $_GET['urlback'] : 'https://app.cash-flag.com' ;
// $urlback = 'https://app.cash-flag.com';
...
switch ($ruta) {
    case 'giftcard':
        $urlok = 'https://app.cash-flag.com/php/exitocompragiftcards.php?url='.$urlback.'&registro='.$registro;
        // $urlok = 'exitocompragiftcards.php?hash='.$hash.'&url='.$urlback;
        $urlcb = $urlback;
        // $urlok = 'https://app.cash-flag.com/php/exitocompragiftcards.php?orden='.trim($orden).'&orden='.trim($monto);
        break;
    case 'prepago':
        $urlok = 'https://app.cash-flag.com/php/exitocompraprepago.php?url='.$urlback.'&registro='.$registro;
        // ?card='.$card;
        $urlcb = $urlback;
        // $urlok = 'https://app.cash-flag.com/php/exitocompragiftcards.php?orden='.trim($orden).'&orden='.trim($monto);
        break;
...
$pagoFlashRequestData = array(
    ...
    "additional_parameters" => array(
            ...
            "url_ok_request" => "https://app.cash-flag.com/php/procesapagoenlinea.php" // en esta url debes verificar la transaccion

            // "url_ok_redirect" =>'https://app.corporacionmanna.com/plataforma/oficina/exitotc.php?orden='.trim($orden), // en esta url le muestas a tu cliente que el pago fue exitoso
            // "url_ok_request" => "https://app.corporacionmanna.com/plataforma/oficina/procesarpagoenlinea.php" // en esta url debes verificar la transaccion
	)
);
